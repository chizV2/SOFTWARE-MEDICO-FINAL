...
class ComunicacionMedicoActivity : AppCompatActivity() {
    ...
    private lateinit var buttonVerificarUsuarios: Button
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_comunicacion_medico)
    ...
    private fun inicializarVistas() {
        ...
        buttonVerificarUsuarios = findViewById(R.id.button_verificar_usuarios)
    ...
    private fun configurarEventos() {
        ...
        buttonVerificarUsuarios.setOnClickListener {
    ...
    private fun mostrarInformacionMedico() {
        try {
            ...
            if (sesion != null) {
                ...
                textViewTitulo.text = "💬 Comunicación Médica"
                textViewSubtitulo.text = "Médico: ${medico.obtenerNombre()}\n\nSelecciona con quién quieres conversar:"
                
                // Obtener ID del médico de manera segura
                medicoId = obtenerIdUsuario(medico.obtenerEmail())
            } else {
                mostrarMensaje("Error: No hay sesión activa")
                finish()
            }
        } catch (e: Exception) {
            mostrarMensaje("Error al mostrar información del médico: ${e.message}")
            e.printStackTrace()
        }
    }
    ...
    private fun cargarConversaciones() {
        try {
            ...
            // Filtrar para no incluir al médico actual
    ...
    private fun cargarMensajesConversacion(remitente: String) {
        try {
            ...
            if (partes.size >= 2) {
                ...
                if (destinatarioId > 0) {
                    val mensajesRecibidos = dbHelper?.obtenerMensajesRecibidos(medicoId) ?: emptyList()
                    val mensajesEnviados = dbHelper?.obtenerMensajesEnviados(medicoId) ?: emptyList()
                    ...
                    mensajesConversacion.forEach { mensajeDb ->
                        try {
                            ...
                            val esMio = remitenteId == medicoId
            ...
            if (texto.isNotEmpty() && conversacionActual.isNotEmpty()) {
                ...
                if (partes.size >= 2) {
                    ...
                    if (destinatarioId > 0) {
                        ...
                        val medico = GestorAutenticacion.obtenerInstancia().obtenerSesionActual()?.usuario
                        val mensajeId = dbHelper?.insertarMensaje(
                            medicoId,
                            medico?.obtenerNombre() ?: "Médico",
                            "MEDICO",
                            destinatarioId,
                            nombreDestinatario,
                            rolDestinatario,
    ...
    private fun verificarUsuariosRegistrados() {
        try {
            val todosUsuarios = dbHelper?.obtenerTodosLosUsuarios() ?: emptyList()
            val pacientes = dbHelper?.obtenerUsuariosPorRol("PACIENTE") ?: emptyList()
            val medicos = dbHelper?.obtenerUsuariosPorRol("MEDICO") ?: emptyList()
            val administrativos = dbHelper?.obtenerUsuariosPorRol("ADMINISTRADOR") ?: emptyList()
            ...
            mensaje.append("Total: ${todosUsuarios.size}\n")
            mensaje.append("Pacientes: ${pacientes.size}\n")
            mensaje.append("Médicos: ${medicos.size}\n")
            mensaje.append("Administrativos: ${administrativos.size}\n\n")
            
            todosUsuarios.forEach { usuario ->
                val id = usuario["id"] as? Long ?: 0
                val email = usuario["email"] as? String ?: ""
                val nombre = usuario["nombre_completo"] as? String ?: ""
                val rol = usuario["rol"] as? String ?: ""
                val activo = usuario["activo"] as? Long ?: 0
                
                mensaje.append("ID: $id, Email: $email\n")
                mensaje.append("Nombre: $nombre\n")
                mensaje.append("Rol: $rol, Activo: $activo\n")
                mensaje.append("---\n")
            }
            
            mostrarMensaje(mensaje.toString())
        } catch (e: Exception) {
            mostrarMensaje("Error al verificar usuarios: ${e.message}")
            e.printStackTrace()
        }
    }
